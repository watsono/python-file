...
def main():
    ...
    while i <= 49:
        ...
        for book_information in books_information:
            ...
            db = pymysql.connect("47.101.215.215", "admin", "123456", "douban")
            cursor = db.cursor()
            insert_sql = ("REPLACE INTO douban_tbl(douban_url,douban_name,douban_info,douban_pf,douban_jian)" "VALUES(%s,%s,%s,%s,%s)")
            data_sql = (href,book_name,book_info,book_pf,book_jian)

            cursor.execute(insert_sql,data_sql)
            db.commit()
        j += 1
